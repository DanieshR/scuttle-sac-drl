...
def get_scan_count():
    # Read the real native SCUTTLE model (the one actually spawned via model://scuttle).
    # Link-name-agnostic: find the ray/lidar sensor wherever it lives.
    tree = ET.parse(os.getenv('DRLNAV_BASE_PATH') + '/scuttle_classic/models/scuttle/model.sdf')
    root = tree.getroot()
    for sensor in root.iter('sensor'):
        ray = sensor.find('ray')
        if ray is not None:
            return int(ray.find('scan').find('horizontal').find('samples').text)
    raise RuntimeError('no ray/lidar sensor found in scuttle model.sdf')

def get_simulation_speed(stage):
    tree = ET.parse(os.getenv('DRLNAV_BASE_PATH') + '/src/turtlebot3_simulations/turtlebot3_gazebo/worlds/turtlebot3_drl_stage' + str(stage) + '/scuttle.model')
    root = tree.getroot()
