...
NO_GOAL_SPAWN_MARGIN = 0.5 # meters away from any wall
class DRLGazebo(Node):
    def __init__(self):
        ...
        ************************************************************"""
        ...
        # Locate the goal-marker model via ament (robust to colcon --symlink-install,
        # where realpath(__file__) resolves into src/ and the old string-replace hack failed).
        ...
        ************************************************************"""
        ...
        # Latched (TRANSIENT_LOCAL) so a late-subscribing environment node still
        # receives the initial goal — fixes the flaky startup "Waiting for new goal" race.
        self.goal_pose_pub = self.create_publisher(Pose, 'goal_pose', QoSProfile(depth=10, durability=DurabilityPolicy.TRANSIENT_LOCAL))
    ...
    *******************************************************************************"""
    ...
    def goal_is_valid(self, goal_x, goal_y):
        if goal_x > ARENA_LENGTH/2 - NO_GOAL_SPAWN_MARGIN or goal_x < -ARENA_LENGTH/2 + NO_GOAL_SPAWN_MARGIN or goal_y > ARENA_WIDTH/2 - NO_GOAL_SPAWN_MARGIN or goal_y < -ARENA_WIDTH/2 + NO_GOAL_SPAWN_MARGIN:
            return False
