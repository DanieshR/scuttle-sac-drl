...
class DRLEnvironment(Node):
    def __init__(self):
        ...
        ************************************************************"""
        ...
        # subscribers
        # Latched QoS to match the gazebo_goals publisher — guarantees the initial
        # goal is received even if this node subscribes after it was published.
    ...
    *******************************************************************************"""
    ...
    def scan_callback(self, msg):
        ...
        for i in range(NUM_SCAN_SAMPLES):
                raw = float(msg.ranges[i])
                if math.isinf(raw) or math.isnan(raw):    # INF/NaN fix: clean before normalize to prevent network collapse
                    raw = LIDAR_DISTANCE_CAP
                self.scan_ranges[i] = numpy.clip(raw / LIDAR_DISTANCE_CAP, 0, 1)
