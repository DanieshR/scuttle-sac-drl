...
class DRLEnvironment(Node):
    ...
    *******************************************************************************"""
    ...
    def scan_callback(self, msg):
        # Clean full scan: replace inf/NaN with cap value
        n_full = len(msg.ranges)
        cleaned = []
        for r in msg.ranges:
            val = float(r)
            if math.isinf(val) or math.isnan(val):
                val = REAL_LIDAR_DISTANCE_CAP
            cleaned.append(val)

        # Downsample into REAL_N_SCAN_SAMPLES buckets using per-bucket minimum (nearest obstacle)
        bucket_size = max(1, n_full // REAL_N_SCAN_SAMPLES)
        self.obstacle_distance = 1
        for i in range(REAL_N_SCAN_SAMPLES):
            start = i * bucket_size
            end = start + bucket_size
            bucket_min = min(cleaned[start:end]) if end <= n_full else (cleaned[start] if start < n_full else REAL_LIDAR_DISTANCE_CAP)
            self.scan_ranges[i] = numpy.clip((bucket_min - REAL_LIDAR_CORRECTION) / REAL_LIDAR_DISTANCE_CAP, 0, 1)
