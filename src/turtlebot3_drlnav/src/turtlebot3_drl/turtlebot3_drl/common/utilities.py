...
import torch
import numpy
from ..common.settings import REWARD_FUNCTION, COLLISION_OBSTACLE, COLLISION_WALL, TUMBLE, SUCCESS, TIMEOUT, RESULTS_NUM, GAZEBO_HARMONIC
...
def pause_simulation(agent_self, real_robot):
    if real_robot or GAZEBO_HARMONIC:
        return
...
def get_scan_count():
    # SCUTTLE LD19 T-Mini Plus: 360 samples, full 360° (-π to π)
    return 360

def get_simulation_speed(stage):
    # Gazebo Harmonic runs at real-time factor 1 (no speed override file)
    return 1