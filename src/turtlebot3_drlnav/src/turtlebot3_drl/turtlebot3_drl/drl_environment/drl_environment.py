...
class DRLEnvironment(Node):
    def __init__(self):
        ...
        # Odom offset — Gazebo Harmonic diff-drive odom does not reset on world reset.
        # Capture the accumulated odom at each episode start and subtract it so the
        # robot always appears at (0,0, yaw=0) in the coordinate frame used for
        # goal distance/angle computation.
        self.odom_x_offset = 0.0
        self.odom_y_offset = 0.0
        self.odom_yaw_offset = 0.0
        self.odom_needs_reset = False  # set True on new goal, cleared on first odom
        self.success_no_reset = False  # skip odom reset and stop when success continues episode
        ...
        ************************************************************"""
        ...
        self.goal_pose_sub = self.create_subscription(Pose, self.goal_topic, self.goal_pose_callback, latch_qos)
    ...
    *******************************************************************************"""
    ...
    def goal_pose_callback(self, msg):
        ...
        if self.success_no_reset:
            # Robot continuing from current position — odom frame is already correct
            self.success_no_reset = False
        else:
            # After fail reset, recapture origin so robot appears at (0,0)
            self.odom_needs_reset = True
        self.new_goal = True
    ...
    def odom_callback(self, msg):
        ...
        _, _, raw_yaw = util.euler_from_quaternion(msg.pose.pose.orientation)
        if self.odom_needs_reset:
            # Capture the current accumulated odom as the episode origin.
            self.odom_x_offset = raw_x
            self.odom_y_offset = raw_y
            self.odom_yaw_offset = raw_yaw
            self.robot_x_prev, self.robot_y_prev = 0.0, 0.0
            self.odom_needs_reset = False
        self.robot_x = raw_x - self.odom_x_offset
        self.robot_y = raw_y - self.odom_y_offset
        yaw = raw_yaw - self.odom_yaw_offset
    ...
    def stop_reset_robot(self, success):
        self.cmd_vel_pub.publish(Twist())  # stop robot (upstream behavior)
        self.episode_deadline = Infinity
        self.done = True
        ...
        if success:
            ...
            self.success_no_reset = True  # skip odom recapture — robot stays at current arena position
